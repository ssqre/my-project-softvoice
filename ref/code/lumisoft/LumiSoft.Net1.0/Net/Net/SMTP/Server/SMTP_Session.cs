...
namespace LumiSoft.Net.SMTP.Server
{
	...
	{				
        ...
        internal SMTP_Session(string sessionID,SocketEx socket,BindInfo bindInfo,SMTP_Server server) : base(sessionID,socket,bindInfo,server)
        {	        
            m_pServer      = server;
			...
			try{	
				...
				if(oArg.Validated){
					...
					if(m_pServer.GreetingText.Length > 0){
						...
						this.Socket.WriteLine("220 " + m_pServer.HostName + " SMTP Server ready");
					}
		...
		private void HELO(string argsText)
		{
			...
			*/
			...
			this.Socket.BeginWriteLine("250 " + m_pServer.HostName + " Hello [" + this.RemoteEndPoint.Address.ToString() + "]",new SocketCallBack(this.EndSend));
			...
			switch(param[0].ToUpper())
			{
				...
				case "DIGEST-MD5":
					...
					*/
					...
					this.Socket.WriteLine("334 " + AuthHelper.Base64en(AuthHelper.Create_Digest_Md5_ServerResponse(realm,nonce)));
			...
			*/
			...
			if(param.Length > 0 && param.Length < 3){				
				if(Core.IsNumber(param[0])){
                    ...
                    // BDAT comman is dummy, after commandline binary data is atonce follwed,
