...
namespace LumiSoft.Net.FTP.Server
{
	...
	public class FTP_Session : SocketServerSession
	{
        ...
        internal FTP_Session(string sessionID,SocketEx socket,BindInfo bindInfo,FTP_Server server) : base(sessionID,socket,bindInfo,server)
        {
            m_pServer = server;
		...
		private void StartSession()
		{
			...
			if(m_pServer.OnValidate_IpAddress(this.LocalEndPoint,this.RemoteEndPoint)){
				...
				this.Socket.WriteLine("220 " + m_pServer.HostName + " FTP server ready");
