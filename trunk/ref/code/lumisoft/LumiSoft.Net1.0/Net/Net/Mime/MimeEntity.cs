		...
		internal bool Parse(Stream stream,String toBoundary)
		{
			...
			if((this.ContentType & MediaType_enum.Multipart) != 0){
				...
				if(boundaryID == null){
                    ...
                    StreamHelper reader = new StreamHelper(stream,4000,false);
					byte[] lineData = new byte[4000];					
                    int readedCount = reader.ReadLine(lineData);
					while(readedCount > 0){
						string line = System.Text.Encoding.Default.GetString(lineData,0,readedCount);
						if(line.StartsWith("--" + boundaryID)){
							break;
						}
						
						readedCount = reader.ReadLine(lineData);
					}
