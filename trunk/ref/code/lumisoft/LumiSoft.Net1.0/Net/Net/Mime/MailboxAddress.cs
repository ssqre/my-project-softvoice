		...
		public static MailboxAddress Parse(string mailbox)
		{
			...
			*/
			...
			if(mailbox.IndexOf("<") > -1 && mailbox.IndexOf(">") > -1){
				name = TextUtils.UnQuoteString(mailbox.Substring(0,mailbox.LastIndexOf("<")));
                emailAddress = mailbox.Substring(mailbox.LastIndexOf("<") + 1, mailbox.Length - mailbox.LastIndexOf("<") - 2);
			}
			else{
