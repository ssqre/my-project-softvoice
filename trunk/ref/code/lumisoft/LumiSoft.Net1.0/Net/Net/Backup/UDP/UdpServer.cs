...
namespace LumiSoft.Net.UDP
{
    ...
    public delegate void PacketReceivedHandler(UdpPacket_eArgs e);
    ...
    public class UdpServer
    {
        ...
        private UdpProcessMode           m_ProcessMode      = UdpProcessMode.Sequential;
            ...
            if(m_pBindings != null){
                ...
                foreach(Socket socket in m_pSockets){
                    if(((IPEndPoint)socket.LocalEndPoint).AddressFamily == AddressFamily.InterNetwork){
                        if(!((IPEndPoint)socket.LocalEndPoint).Equals(IPAddress.Loopback)){
        ...
        public void SendPacket(byte[] packet,int offset,int count,IPEndPoint remoteEP)
        {
            ...
            SendPacket(null,packet,offset,count,remoteEP);
        }
        ...
        internal void SendPacket(Socket socket,byte[] packet,int offset,int count,IPEndPoint remoteEP)
        {
        ...
        private void ProcessQueuedPackets()
        {
            while(m_IsRunning){
                try{
                    // There is no packets to process.
                    if(m_pQueuedPackets.Count == 0){
                        ...
                        if(m_ProcessMode == UdpProcessMode.Sequential){
        ...
        {            
            if(this.PacketReceived != null){
                this.PacketReceived(new UdpPacket_eArgs(this,packet.Socket,packet.RemoteEndPoint,packet.Data));
            }
        }
