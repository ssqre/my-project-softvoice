...
namespace LumiSoft.Net.SIP.Stack
{
    ...
    {        
        ...
        /// <summary>
        /// Gets contact host for sip or sips. Returns host suitable for Contac: header field.
        /// </summary>
        /// <param name="sip_sips">If true then sip, otherwise sips.</param>
        /// <returns>Returns contact host.</returns>
        /// <exception cref="ArgumentException">Is raised when SIP stack has no suitable listening point for sip or sips.</exception>
        public string GetContactHost(bool sip_sips)
        {
            if(string.IsNullOrEmpty(this.HostName)){
                foreach(BindInfo bind in m_pBinds){
                    // sip
                    if(sip_sips && !bind.SSL){
                        return bind.IP.ToString() + ":" + bind.Port;
                    }
                    // sips
                    else{
                        return bind.IP.ToString() + ":" + bind.Port;
                ...
                throw new ArgumentException("No suitable contact host sip or sip listening point !");
            }
            else{
        ...
        /// <returns></returns>
        public SIP_Target[] GetTargets(string uri)
        {
            return null;
        }
        */
        ...
        {
            get{ return m_MinSessionExpires; }
