    ...
    internal class SIP_TransportLayer
    {
                ...
                if(System.Text.Encoding.UTF8.GetString(data,0,10).ToUpper().StartsWith("SIP")){
                    try{                        
                        ...
                        if(m_pSipStack.Logger != null){
                            m_pSipStack.Logger.AddDebug("Skipping message, parse error: " + x.ToString());
                        }
                    }
                }
                // Request
                            ...
                            foreach(SipListeningPoint listeningPoint in m_pListeningPoints){
                                if(listeningPoint.Protocol == BindInfoProtocol.UDP){
                                    if(localEndPoint == null || listeningPoint.LocalEndPoint.Equals(localEndPoint.EndPoint)){
                                        ...
                                        else if(listeningPoint.LocalEndPoint.Address.Equals(IPAddress.Any)){
                                            // Get first IP
        ...
        public void SendResponse(SocketEx socket,IPEndPoint remoteEndPoint,SIP_Response response)
        {
            ...
            */
            ...
            // TODO: Do we need remote end point ? If we always fill received and rport ?
            ...
            try{                
                ...
                if(socket == null){
                    ...
                    // UDP
                        ...
                        foreach(SipListeningPoint listeningPoint in m_pListeningPoints){
                            if(listeningPoint.Protocol == BindInfoProtocol.UDP){
                                ...
                                // Log
                                m_pSipStack.Logger.AddWrite(data.Length,"Sent (" + data.Length + " bytes): " + listeningPoint.Socket.LocalEndPoint.ToString() + " -> " + remoteEndPoint.ToString() + "\r\n<begin>\r\n" + System.Text.Encoding.UTF8.GetString(data) + "<end>\r\n");
                                return;
                            }
                        }
                    ...
                    // TCP
                        ...
                        // Log
                        m_pSipStack.Logger.AddWrite(data.Length,"Sent (" + data.Length + " bytes): " + pipe.LocalEndPoint.ToString() + " -> " + remoteEndPoint.ToString() + "\r\n<begin>\r\n" + System.Text.Encoding.UTF8.GetString(data) + "<end>\r\n");
                    }
                    // TCP TLS            
